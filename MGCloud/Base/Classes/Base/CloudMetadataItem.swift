...
extension Dictionary<String, CloudMetadataItem> {
    public func isFileExist(at:URL)->Bool {
        let item = self[at.path]
        return item != nil
    }
    ...
    public mutating func deleteSubItemAtUrl(url:URL)->Bool {
        self[url.path] = nil
        return false
    }
    
    public mutating func addSubItem(subItem:CloudMetadataItem)->Bool {
        self[subItem.url.path] = subItem
        return true
    }
    ...
    func changeRootPath(rootPath:String) {
        ...
        self.map { key,val in
            ...
            var coms = url.path.split(separator: "/")
            
            if !coms.first!.isEmpty {
                coms.removeFirst()
            }
            var curUrl = URL.init(string: urlStr)!
            curUrl.appendingPathComponent(rootPath)
            for item in coms {
                curUrl = curUrl.appendingPathComponent(String(item))
            }
            val.url = curUrl
            return (curUrl.path,val)
        }
    }
}
